...
export function getFirstDayOfMonth(date: Date | string): string {
  ...
  return formatDateToMMDDYYYY(firstDay);
}
 ...
 * @returns Date string in MM/DD/YYYY format
 */
export function getLastDayOfMonth(date: Date | string): string {
  const dateObj = typeof date === 'string' ? parseDate(date) : date;
  const lastDay = new Date(dateObj.getFullYear(), dateObj.getMonth() + 1, 0);
  return formatDateToMMDDYYYY(lastDay);
}
 ...
 */
export function getMonthsBetween(startDate: string, endDate?: string): Array<{month: number, year: number}> {
 ...
 */
export function calculateINVDate(admissionDate: string): string {
  const date = parseDate(admissionDate);
 ...
 */
export function calculateHUV1Date(admissionDate: string): string {
  const date = parseDate(admissionDate);
 ...
 */
export function addDaysToDate(baseDate: string, daysToAdd: number): string {
