...
test.describe.serial('HOPE Visit - No Preferences with Mild Symptoms', () => {
  ...
  test('Step 02: Search for Patient with Mild symptoms', async () => {
    test.setTimeout(120000);
  ...
  test('Step 04: Perform INV Visit - No Preferences, Mild Symptoms', async () => {
  ...
  test('Step 07: Validate Complete HOPE Preview Report', async () => {
    ...
    await pages.hopePreview.validateCompleteHOPEPreview(HOPE_NO_MILD_CONFIG);

    console.log('Step 7 Complete: HOPE Preview validated');
  });

  test('Step 08: Validate Skin Condition Alert', async () => {
    ...
    if (previewContent?.includes('wound') || previewContent?.includes('skin')) {
      ...
      console.log('Skin alert verification skipped (selector may vary)');
    }
    ...
    console.log('Symptoms: Mild Impact');
